...
9. **Premium sports-site features** (approved by Chris) — IN PROGRESS (PR #42):
   ...
   - DONE: "Book Dina" page (`/roxjaya/book-dina/`) — formats (squad sim, clinic, race-day, talk, brand day), "Where she'll be" from `roxjaya/data/appearances.json` (confirmed only, past rows hide), inquiry form → `/api/capture-email` with `profile.source=book-dina` so it lands in People with the other leads. No public availability calendar by design.
   - DONE: Home-page "Athletes say" block from `roxjaya/data/testimonials.json` — hidden until Dina adds a real quote (no placeholder copy shipped).
