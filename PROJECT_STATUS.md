...
1. **Tenant + home page** — DONE (PR #42). Add `roxzone` to `data/tenants.json` (IDR, brand, SEO meta, `vertical: "coaching"`); promote the chosen layout to `/roxzone/` home; retire the unused layout to a comparison page. Global touch: tenant loader must tolerate a non-pet vertical (skip petType/safety validators). _Local_ otherwise.
...
9. **Premium sports-site features** (approved by Chris) — IN PROGRESS (PR #42):
   - **Indonesia Results board** — DONE: `/roxzone/results/` ranks Indonesian finishers by time for Women, Men, Women Doubles, Men Doubles, Mixed, Relay; data lives in `roxzone/data/results.json` (one row per finish; page sorts, empty divisions show "send us your time"); name search; Dataset/FAQ/Breadcrumb JSON-LD; source credited (official Hyrox results) + correction/removal route via Ask Dina/IG. Seeded with Dina's confirmed results only. **Data blocker:** results.hyrox.com and hyresult.com both return 403 to automated access — do not scrape around it; populate manually from the official boards (copy Indonesian finishers into the JSON) or ask Hyrox/hyresult for an export.
   ...
   - **Per-plan SEO pages** — DONE: `/roxzone/warrior-program/`, `/roxzone/hybrid-coaching-jakarta/`, `/roxzone/elite-race-prep/` (who it's for, included, typical week, FAQ; Service+Offer/Breadcrumb/FAQ JSON-LD; checkout button + seats on page; product canonicalPaths point here; linked from plan cards + sitemap).
   ...
   - **Taper Zone** — DONE: `/roxzone/taper-zone/` (why taper, two weeks out, race-week day-by-day, what to keep sharp, sleep + travel, race-week checklist; links Fuel Zone / Run Zone / Split analyser; Article + FAQ + Breadcrumb JSON-LD; Dina to approve).
   ...
   - **HYROX Certified Coach badge** — DONE (provisional): hero, bio, plan cards, guide bylines and Person JSON-LD say "HYROX Certified Coach". **Dina to confirm the exact certificate title** (Chris: "pretty sure it's the certified coach"); it's plain text in the roxzone HTML files, one find-and-replace to correct.
   ...
   - **Start-here quiz** — DONE: submitting the Ask Dina questionnaire shows a "Your zone" card (Program / Hybrid / Elite + one-line reason, CTA to `#plans`, recommended card outlined) and stores `recommended_plan` on the lead profile. Rules live in `roxzone/app.js` (`recommend()`): elite for podium goal or racers <16 weeks out; Hybrid for Jakarta-area first-timers/PB/doubles/pregnancy; Program otherwise or for 2 days/week.
   - Next: per-plan SEO pages, Athlete Mum page (after Feb), station-split comparison once verified splits exist.
