...
2. **Recurring billing** — DONE (PR #42): `kind: "service"` products with `billing.interval`; checkout picks `mode: "subscription"` when every item is recurring (mixed carts rejected); `subscriptions` table (migration 005) + renewals land as paid orders; Program/Hybrid buttons live, Elite stays enquiry-only. Untested against a real Stripe account yet — needs `STRIPE_SECRET_KEY` + webhook secret and the migration run. Original spec: Stripe Checkout `mode: "subscription"` on the shared engine + `customer.subscription.created/updated/deleted` and `invoice.paid/payment_failed` webhooks; plans as tenant-scoped price IDs; orders/customers stitched exactly like one-time orders. Global change (also unlocks "membership tiers" for the pet sites). Guardrail: server-authoritative prices, integer minor units.
