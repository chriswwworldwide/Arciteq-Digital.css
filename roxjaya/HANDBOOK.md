...
| Add a sim / hybrid event       | Edit `roxjaya/data/scene.json` (or tell me and I'll add it)  | One object per event: `type` race/sim/brand/venue, `mode` race/watch/both, `date` only if the organiser has published it (else leave `""` and describe in `when`), organiser `link`. Shows on /roxjaya/jakarta-hybrid-scene/.                                                                                                                           |
...
| Approve a sponsor              | Today → "Waiting for your OK" → check brand + link → Approve | A sponsor books and pays on /roxjaya/advertise/; you get a "Sponsor" card (their logo, link, one-liner — all editable). Approve = live within a minute; Ignore = stays off the site (refund them in Stripe by hand). Renewals extend automatically; when they stop paying the logo drops off by itself. Clicks per sponsor: Traffic → "Sponsor clicks". |
| Where you'll be (Book Dina)    | `roxjaya/data/appearances.json`                              | One row per confirmed public appearance: `date`, `title`, `where`, `kind` sim/clinic/race/talk/brand. Past ones hide. Inquiries arrive in People tagged `book-dina`.                                                                                                                                                                                    |
| Client quotes on the home page | `roxjaya/data/testimonials.json`                             | Add a real quote (their words, their OK) with `quote`, `name`, `detail`. The "Athletes say" section appears automatically once there is one; up to three show.                                                                                                                                                                                          |
| Pull a sponsor early           | `roxjaya/data/partners.json` → delete their row              | Refund by hand in Stripe. Slots: `results` (results + splits pages), `planner` (race planner + Jakarta), `partner` (footer strip everywhere).                                                                                                                                                                                                           |
...
sponsor bookings and payments (you only approve or ignore),
