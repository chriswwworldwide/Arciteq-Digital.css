...
9. **Premium sports-site features** (approved by Chris) — IN PROGRESS (PR #42):
   ...
   - Queue: Jakarta Hybrid Scene page; automation (Asia event-date puller with approve queue, fail-loud results pull, weekly "what needs Dina" digest with Coach's desk deep links); curated Hyrox Asia news feed (headline + short rewritten summary + source link, few per week, published only when Dina taps approve from the digest — deliberately not a bulk auto-rewriter); sponsorship rails (ad-slot products via existing checkout, `partners.json`, self-serve `/roxjaya/advertise/`, approve/decline, click reporting); Athlete Mum page (after Feb).
